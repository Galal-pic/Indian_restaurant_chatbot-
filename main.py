...
async def handle_request(request: Request):
        ...
        intent_hadelr_dict={'tracking.order-context:ongoing-tracking':track_order,
                            'order.add - context:ongoing-order':add_to_order,
                             'order.complete-context:ongoing-order':complete_order,
                             'order.remove - context: ongoing-order':remove_from_order
        }
        return  intent_hadelr_dict[intent](parameters,session_id)
...
def complete_order(parameters: dict, session_id: str):
    if session_id not in inprogress_order:
        ...
        if order_id == -1:
            fulfillment_text = "Sorrrry, I couldn't process your order due to a backend error. " \
                               "Please place a new order again"
        else:
            order_total = df_helper.get_total_order_price(order_id)
...
def save_to_db(order: dict):
    next_order_id = df_helper.get_next_order_id()
    # print('************************************')
    # print(order)
    # Insert individual items along with quantity in orders table
    for food_item, quantity in order.items():
        rcode = df_helper.insert_order_item(food_item,quantity,next_order_id )

        if rcode == -1:
            return -1

    # Now insert order tracking status
    df_helper.insert_order_tracking(next_order_id, "in progress")
...
def track_order(parameters : dict,session_id:str):
        ...
        order_status=df_helper.get_order_status(order_id)
        if order_status:
